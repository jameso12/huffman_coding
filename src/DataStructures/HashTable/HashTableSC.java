...
public class HashTableSC<K, V> implements Map<K, V> {
	...
	private void rehash() {
		...
		// Create new list
		List<BucketNode<K, V>>[] temp = new LinkedList[this.size()*2];
		// initialize buckets
		for (int i = 0; i < temp.length; i++)
			temp[i] = new LinkedList<BucketNode<K, V>>();
		for(List<BucketNode<K, V>> e: buckets)
		{
			for(BucketNode<K, V> bn : e)
			{
				temp[hashFunction.hashCode(bn.getKey()) % temp.length].add(0,bn);// re-hash
			}
		}
		this.buckets = temp; // buckets now points to new hash-table		
	}
